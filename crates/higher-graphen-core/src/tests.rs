use crate::{
    Capability, CapabilityOperation, CapabilityStatus, Confidence, ContextRestriction, CoreError,
    CorrespondenceCell, CorrespondenceKind, CorrespondenceParticipant, CorrespondencePolarity,
    CorrespondenceValidationCode, CriterionDirection, CriterionValue, Derivation,
    DerivationFailureMode, Description, DifferenceKind, DifferenceSeverity, DifferenceWitness,
    DifferingStructure, EquivalenceClaim, EquivalenceCriterion, EquivalenceKind, EquivalenceScope,
    GluingAttempt, GluingResult, Id, InferenceRule, LifecycleStatus, NormalizedClaim, ObjectRef,
    OrderType, OverlapWitness, OverlapWitnessKind, ParticipantRef, PayloadRef, Policy,
    PolicyApplicability, PolicyKind, PolicyRule, PolicyStatus, PreservationReport, Provenance,
    QuotientEffect, Reachability, RequiredReview, ReviewRequirement, ReviewStatus, Scenario,
    ScenarioChanges, ScenarioKind, ScenarioStatus, SchemaCompatibility, SchemaMapping,
    SchemaMappingKind, SchemaMorphism, SchemaVerification, Scope, Severity, SharedStructure,
    SourceKind, SourceRef, Tradeoff, Valuation, ValuationCriterion, ValuationValue,
...
fn severity_and_review_status_have_stable_values_and_order() {
    ...
    assert_eq!(
        ReviewStatus::try_from("candidate").unwrap(),
        ReviewStatus::Candidate
    );
    assert_eq!(ReviewStatus::Candidate.as_str(), "candidate");
    assert!(!ReviewStatus::Candidate.has_review_action());
    assert_eq!(
