use super::*;
use higher_graphen_space::{Cell, ComplexType, Space};
...
#[test]
fn reports_uncovered_boundary_and_unsupported_dimension_findings() {
    let mut store = seeded_store();
    store
        .insert_cell(Cell::new(id("point-a"), id("space-a"), 0, "point"))
        .expect("insert point a");
    ...
    assert_eq!(
        summary.findings[0].finding_type,
        TopologyFindingKind::ExternalBoundaryCell
    );
    assert_eq!(
        summary.findings[0].obstruction_type.as_deref(),
        Some(UNCOVERED_REGION_OBSTRUCTION_TYPE)
    );
    assert_eq!(
        summary.findings[1].finding_type,
        TopologyFindingKind::UnsupportedDimension
    );
}
